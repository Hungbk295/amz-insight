...
const crawl = async (page, crawlInfo) => {
    return classify(crawlInfo["link"])[1](page, crawlInfo)
}
...
export const run = async (queueUrl, workerName) => {
    ...
    while (true) {
        try {
            ...
            if (data.Messages) {
                for (const msg of data.Messages) { // why for in single array
                    ...
                    try {
                        ...
                        await deleteSqsMessage(queueUrl, msg.ReceiptHandle);// delete by ReceiptHandle / why not by MessageId|MD5OfBody:
                    } catch (e) {
