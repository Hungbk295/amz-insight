...
import {
	getConfigBySupplierId,
	INTERNAL_SUPPLIER_IDS,
	SUPPLIERS,
} from '../../config/suppliers.js'
...
import {
	...
	COUNT_RECEIVE_MESSAGE,
	LIMIT_REMAINING_MESSAGE,
} from '../../config/app.js'
		...
		if (data.Messages) {
			...
			for (const msg of data.Messages) {
				...
				await client.updateClientStatus(
					workerName,
					client.CLIENT_STATUS.WORKING
				)
				const supplierId = task['supplierId']
				task['keyword'] = keywords.find(
					keyword => keyword.id === task['keywordId']
				)
				const browser = await getContext(getConfigBySupplierId(supplierId))
				const page = await (INTERNAL_SUPPLIER_IDS.includes(
					getConfigBySupplierId(supplierId).id
				)
					? browser.pages()[0]
				...
				try {
					...
					await createSqsMessages(
						process.env.QUEUE_RESULTS_URL,
						_.chunk(resultData, 10)
					)
					if (
						SUPPLIERS_WITH_DETAIL_PRICE.map(item => item.id).includes(
							supplierId
						)
					)
						await crawlers[supplierId].generateHotelDetailTasks(
							resultData,
							task['keyword']
						)
					await deleteSqsMessage(queueUrl, msg.ReceiptHandle)
				...
				await browser.close()
				const end = Date.now()
				if (checkSqsPeriodOfTime(start, end, data.Messages, 'crawl top'))
					break
			}
		}
