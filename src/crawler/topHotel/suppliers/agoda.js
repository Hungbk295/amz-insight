...
export const crawl = async (page, crawlInfo) => {
    ...
    for (const info of hotel_infos){
        const hotel = {};
        try {
            const hotel_name = await (await info.$(`//h3[contains(@data-selenium,'hotel-name')]`)).innerText();
            const hotel_price = await (await info.$(`//span[contains(@data-selenium,'display-price')]`)).innerText();
