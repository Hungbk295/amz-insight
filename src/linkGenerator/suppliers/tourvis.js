...
export class Tourvis {
    generateTopHotelTask(checkIn, checkOut, keywordItem, createdAt) {
        ...
        return {
            link: `hotels?type=${keywordItem['priviaDestType']}&keyword=${encodedKeyword}&id=${keywordItem['priviaDestId']}&in=${checkIn.replaceAll('-', '')}&out=${checkOut.replaceAll('-', '')}&guests=2`,
            checkIn: checkIn,
            checkOut: checkOut,
    ...
    generateHotelDetailTask(checkIn, checkOut, keyword, createdAt, hotelInfo) {
        if (hotelInfo['link']) {
            ...
            const link = hotelInfo['link'] + `?type=${keyword['priviaDestType']}&keyword=${encodedKeyword}&id=${keyword['priviaDestId']}&&in=${checkIn.replaceAll('-', '')}&out=${checkOut.replaceAll('-', '')}&guests=2&pageType=main&h=1`
            return {
                name: hotelInfo['name'],
